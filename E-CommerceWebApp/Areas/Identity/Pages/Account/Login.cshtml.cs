...
using E_CommerceWebApp.Authorization;
...
namespace E_CommerceWebApp.Areas.Identity.Pages.Account
{
    public class LoginModel : PageModel
    {
        ...
        public LoginModel(SignInManager<ApplicationUser> signInManager, ILogger<LoginModel> logger , ICartRepository cartRepository)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    if (cartIdClaim == null)
                    {
                        ...
                        var userCart = _cartRepository.GetUserCart(userIdClaim);

                        // just incase
                        if (userCart == null)
                            // create new cart for user
                            userCart = _cartRepository.CreateUserCart(userIdClaim);

                        // create new claim for the user
                        Claim userCartIDClaim = new Claim(CustomClaims.CartId, userCart.CartId.ToString());
